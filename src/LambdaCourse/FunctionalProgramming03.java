...
//	printRemoveifSartingAorEndsN(l);
	System.out.println();
	lengthSquareDistinctReversed(l);
	System.out.println();
	checkLengthLessThan12(l);
	System.out.println();
	checkInitials(l);
	System.out.println();
	checkLastChar(l);
	...
	//1. way
        ...
        public static void  printUppercase2(List<String> l) {
        	...
        	//use either forEach(Utils::printIntheSameLineWithSpace); or syso
...
//        public static void  printRemoveifSartingAorEndsN (List<String> l) {
...
//        }
      ...
      //7) Create a method which takes the square of the length of every element, prints them distinctly in reverse order
        ...
        public static void  lengthSquareDistinctReversed (List<String> l) {
        	...
        	l.stream().map(String::length).map(Utils::getSquare).distinct().sorted(Comparator.reverseOrder()).forEach(Utils::printIntheSameLineWithSpace);
        	
        }
      //8) Create a method to check if the lengths of all elements are less than 12
       
        public static void checkLengthLessThan12(List<String> l){
      ...
      //10) Create a method to check if ((at least)) one of the elements ending with “R”
        ...
        boolean result = l.stream().anyMatch(t->t.endsWith("C"));
        System.out.println(result);
